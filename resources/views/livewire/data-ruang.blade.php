    ...
    <div class="card mb-3">
            ...
            <table class="table table-hover">
                ...
                <tbody class="table-border-bottom-0">
                    @forelse ($data_ruang as $index => $data)
                        <tr>
                            <td>{{ $data_ruang->firstItem() + $index }}</td>
                            <td>
                            ...
                            <td class="text-center">
                                <div class="d-flex gap-1 justify-content-center">
                                    <a href='{{ url('data-ruang/'.$data->id.'/edit') }}' class="btn btn-warning btn-sm">Edit</a>
                                    <form onsubmit="return confirm('Apakah Anda Yakin?');" action="{{ route('data-ruang.destroy', $data->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
        ...
        <caption>
            <div class="d-flex align-items-center justify-content-center mt-3">
                <small class="pagination-sm">{{ $data_ruang->links() }}</small>
            </div>
        </caption>
    </div>
